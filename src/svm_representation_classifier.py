...
cluster_data_directory = '/cluster/project1/FFPredRNN/data/biological_process/psipred/optimised_go_terms/'

# Load in GO term index mappings
...
# Extract protein psipred features from tar file
tar = tarfile.open(cluster_data_directory + "%s.tar.gz" % (go_term))
tar.extractall(cluster_data_directory)
tar.close()

# Protein train and test splits for a given GO term.
...
for index, protein in enumerate(proteins_train):
    ...
    try:
        ...
        # Load in the LSTM representations
        with open(cluster_data_directory + go_term + '/train/' + protein + '/representation.pkl', 'rb') as f:
...
for index, protein in enumerate(proteins_test):
    ...
    try:
        ...
        # Load in the LSTM representations
        with open(cluster_data_directory + go_term + '/test/' + protein + '/representation.pkl', 'rb') as f:
            representation = pickle.load(f)
...
# Remove the protein feature psipred directory to save memory.
shutil.rmtree(
    '/cluster/project1/FFPredRNN/data/biological_process/psipred/optimised_go_terms/' + go_term)


# Set up training/ testing features and labels
...
# Transform into a single binary-output problem
y_train = y_train[:, go_index]
y_test = y_test[:, go_index]

# Define the hyperparameter search dictionary
param_grid = [
...
# Instantiate cross validation grid search
...
# Train an optimised SVM given the best hyperparameters
if kernel_value == 'rbf':
...
# Make predictions and compute the F1 score
predictions = svc_trained.predict(X_test)
f1 = f1_score(predictions, y_test)

# Save the probabilities
with open('/cluster/project1/FFPredRNN/data/svm_mixed_repr_proba_3.txt', 'a') as f:
...
with open('/cluster/project1/FFPredRNN/data/svm_mixed_hyperparameter_results_3.txt', 'a') as f:
